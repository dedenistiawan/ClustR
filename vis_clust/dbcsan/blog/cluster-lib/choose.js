...
function display_choice(choices, title, next_fn) {
  ...
  // Make the rectangles
  var choice_rects = svg
    .selectAll(".choice_rect")
    .data(choices)
    .enter()
    .append("rect")
    .attr("class", "choice_rect")
    .attr("x", function (d, i) {
      return x(-15 + (30 / 3) * (i % 3));
    })
    .attr("y", function (d, i) {
      return y(5 - 5 * Math.floor(i / 3));
    })
    .attr("rx", 10)
    .attr("ry", 10)
    .attr("width", x(8) - x(0))
    ...
    .on("click", function (d) {
      make_choice(d, next_fn);
    });

  // Make the choice text
  svg
    .selectAll(".choice_text")
    .data(choices)
    .enter()
    .append("text")
